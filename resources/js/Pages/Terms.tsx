...
function BlockContent({ block }: { block: Block }): ReactNode {
    if (block.kind === 'p') {
        return <p className="text-base leading-relaxed text-ink/65">{block.text}</p>;
    }

    if (block.kind === 'sub') {
        return (
            <div>
                <h4 className="mb-3 text-base font-medium text-ink">{block.title}</h4>
                <p className="text-base leading-relaxed text-ink/65">{block.text}</p>
            </div>
        );
    }
        ...
        return (
            <ul className="space-y-3">
                {block.items.map((item) => (
                    <li key={item} className="flex gap-4 text-base leading-relaxed text-ink/65">
    ...
    return (
        <div className="rounded-xl border border-gold/20 bg-gold/5 p-8">
            <div className="mb-3 flex items-center gap-3">
                <AlertTriangle className="h-4 w-4 text-gold-ink" />
                <h4 className="text-[11px] font-semibold uppercase tracking-[0.2em] text-gold-ink">{block.title}</h4>
            </div>
            ...
            <main className="pt-20 xl:pt-24">
                ...
                <section className="mx-auto max-w-7xl px-6 pt-20 md:px-12 md:pt-28">
                    <motion.div initial="initial" animate="animate" variants={staggerContainer} className="max-w-4xl">
                        ...
                        <motion.div
                            variants={fadeIn}
                            className="flex flex-wrap gap-x-8 gap-y-2 text-[11px] uppercase tracking-[0.2em] text-ink/60"
                        >
                ...
                <section className="mx-auto max-w-7xl px-6 py-20 md:px-12">
                    <p className="mb-8 text-[10px] font-semibold uppercase tracking-[0.3em] text-ink/55">
                        Legal &amp; Compliance
                    </p>
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        {legalLinks.map((item) => (
                            ...
                            >
                                <span className="mb-2 block text-sm font-medium text-ink">{item.label}</span>
                                <span className="block text-xs text-ink/62">{item.description}</span>
                            </Link>
                        ))}
                    </div>
                        ...
                        <div className="grid gap-x-10 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
                            {sections.map((section) => (
                                <a
                                    key={section.id}
                                    href={`#${section.id}`}
                                    className="group flex items-baseline gap-4 text-sm text-ink/65 transition hover:text-gold-ink"
                                >
                                    <span className="font-serif text-xs italic text-ink/55 transition group-hover:text-gold-ink">
                                        {section.number}
                                    </span>
                                    {section.title}
                                </a>
                ...
                <div className="mx-auto max-w-7xl px-6 md:px-12">
                    {sections.map((section) => (
                        ...
                        >
                            <div>
                                <span className="mb-3 block font-serif text-2xl italic text-gold-ink">
                                    {section.number}
                                </span>
                                <h3 className="font-serif text-2xl italic leading-snug text-ink">{section.title}</h3>
                            </div>
